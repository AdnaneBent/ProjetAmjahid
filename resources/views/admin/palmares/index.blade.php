...
<br>
 <table class="table table-light">
        ...
        <tbody>
            ...
            <td class="d-flex"><a class="btn btn-primary" href="{{route("palmares.edit",['palmares'=>$palmaress->id])}}"> Modifié</a>
                <form action="{{route('palmares.destroy',['palmares'=>$palmaress->id])}}" method="post">
                @method('DELETE')
                @csrf
                <button type="submit" class="btn btn-danger mx-2">Supprimé</button>
            </form></td>
            </tr>
            @endforeach
        </tbody>
